...
namespace igraph {
	...
	temporary_class<VertexSelector>::type VertexSelector::all() MAY_THROW_EXCEPTION {
		igraph_vs_t _ = igraph_vss_all();
		return ::std::move(VertexSelector(&_, OwnershipTransferNoOwnership));
	}
	
	temporary_class<VertexSelector>::type VertexSelector::adj(const Vertex which, const NeighboringMode mode) MAY_THROW_EXCEPTION {
		igraph_vs_t _;
		igraph_vs_adj(&_, which, (igraph_neimode_t)mode);
		return ::std::move(VertexSelector(&_, OwnershipTransferMove));
	}
	
	temporary_class<VertexSelector>::type VertexSelector::nonadj(const Vertex which, const NeighboringMode mode) MAY_THROW_EXCEPTION {
		igraph_vs_t _;
		TRY(igraph_vs_nonadj(&_, which, (igraph_neimode_t)mode));
		return ::std::move(VertexSelector(&_, OwnershipTransferMove));
	}
	
	temporary_class<VertexSelector>::type VertexSelector::none() MAY_THROW_EXCEPTION {
		igraph_vs_t _ = igraph_vss_none();
		return ::std::move(VertexSelector(&_, OwnershipTransferNoOwnership));
	}
	
	temporary_class<VertexSelector>::type VertexSelector::single(const Vertex which) MAY_THROW_EXCEPTION {
		igraph_vs_t _ = igraph_vss_1(which);
		return ::std::move(VertexSelector(&_, OwnershipTransferNoOwnership));
	}
	
	temporary_class<VertexSelector>::type VertexSelector::vector(const VertexVector& vec, const OwnershipTransfer transfer) MAY_THROW_EXCEPTION {
		igraph_vs_t _;
		switch (transfer) {
			...
			case OwnershipTransferMove: {
				...
				VertexSelector vs = VertexSelector(&_, OwnershipTransferNoOwnership);
	...
	temporary_class<VertexSelector>::type VertexSelector::seq(const Vertex fromID, const Vertex toID) MAY_THROW_EXCEPTION {
	...
	temporary_class<VertexVector>::type VertexSelector::as_vector(const Graph& g) const MAY_THROW_EXCEPTION {
		igraph_vector_t res;
